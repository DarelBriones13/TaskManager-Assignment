let taskList = [];
    ...
    function markTaskCompleted() {
      ...
      const taskIndex = parseInt(completionInput.value) - 1; // Adjusting for 0-based index
      if (Number.isInteger(taskIndex) && taskIndex >= 0 && taskIndex < taskList.length) {
        taskList[taskIndex].completed = true;
        displayTaskList();
        completionInput.value = ''; // Clearing the input field after marking task completed
      } else {
    ...
    function displayTaskList() {
      ...
      if (taskList.length === 0) {
        ...
        taskList.forEach((task, index) => {
          ...
          taskNumber.textContent = index + 1 + '. '; // Task number
          ...
          editButton.classList.add('edit-button'); // Adding class for styling
